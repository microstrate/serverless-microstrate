...
const { constants, deployAPI, mergeObjects } = require('../shared')
...
const runDeploymentStep = fn => {
  ...
  return deployments => {
    ...
    if (deployAPI.isSuccessful(...deployments)) {
  ...
  deploy() {
    ...
    const steps = [
      ...
      ([resourcesDpl]) => {
        ...
        const namespace = collectionResource.properties.subject.split('.')[1]
    ...
    return steps
      .reduce(
        (chain, step) => chain.then(runDeploymentStep(step)),
        Promise.resolve(),
      )
