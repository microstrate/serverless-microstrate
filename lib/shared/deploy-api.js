...
/**
 * @param {object} stack
 * @param {object} [stack.data]
 * @param {Object.<string, any>} stack.data.resources
 * @returns {boolean}
 */
const isSuccessful = stack =>
  stack.data &&
  stack.data.result &&
  Object.values(stack.data.result).every(r => r.status === 'SUCCESS')
  ...
  mergeObjects(deployment, {
    data: mergeObjects(deployment.data, {
      resources: Object.entries(deployment.data.resources).reduce(
        (acc, [key, obj]) =>
          resourceIds.includes(key) ? mergeObjects(acc, { [key]: obj }) : acc,
        {},
      ),
