...
const AWSResourceMapping = {
  'AWS::Lambda::Function': (key, config, provider) =>
    migrateResource(
      key,
      migrateLambdaToFunctionConfig(config),
      provider,
...
const migrateLambdaToFunctionConfig = ({ events: _, ...config }) => {
  /**
   * @type {import('./types').MicroStrateFunctionConfig}
   */
  const result = {
    ...config,
    ...
    //     if (ev.httpApi) {
    //       let method, path
    //       if (typeof ev.httpApi === 'object') {
    //         ;({ method, path } = ev.httpApi)
    //       } else if (typeof ev.httpApi === 'string') {
    //         ;[method, path] = ev.httpApi.split(' ').filter(x => !!x)
    //       }
